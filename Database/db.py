...
class DBConnect():
    ...
    def getStats(self):
        sql = '''
                        SELECT 
                        make,model,trim, avg(Truecar_price),avg(miles), count(trim) as sampleSize
                        FROM 
                        scrapeData_
                        WHERE
                        upc_product_code LIKE '{}'
                        
                        '''.format(str(self.VINcode))
        results = pd.read_sql_query(sql, self.myDB)
        returnObject = {'make': str(results['make']),
                                        'model':str(results['model']),
                                        'trim': str(results['trim']),
                                          'sampleSize': str(results['sampleSize'])}
        return returnObject
