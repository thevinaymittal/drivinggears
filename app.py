    ...
    if request.method == 'POST':
        ...
        if request.form.get('VIN') :
            ...
            #01 AI values
                ...
                try:
                    ...
                    try:
                                ...
                                try:
                                    ...
                                    if ((Avg_Price/20) > Std_Price):
                                        BmarketofAI  = int(int(prediction) - 2*int(results['Std Price']))
                                        AmarketofAI  = int(prediction) 
                                        AvgmarketofAI= int(prediction)     - int(results['Std Price'])
                                    else :
                                        
                                        BmarketofAI  = int(int(prediction) - 2*int(results['Std Price']))
                                        AmarketofAI  = int(prediction) - 1.25*int(results['Std Price'])
                                        AvgmarketofAI= int(prediction)     - 1.75*int(results['Std Price'])
                                    print(datetime.now())
                                    #5
                                    print("caching")
                ...
                BBuyerAsked = max(AmarketofAI,AmarketofStats,int(float(AmarketofVINaudit))) -  int(ldata["stdev"])
                ...
                ASellerAsked= min(BmarketofAI,BmarketofStats,int(float(BmarketofVINaudit)))+  int(ldata["stdev"])

                BBuyerAsked = min(AmarketofAI,AmarketofStats,int(float(AmarketofVINaudit))) -  int(ldata["stdev"])
